...
def run_audit(base_dir: str = "."):
    ...
    load_dotenv()
    logger.info(f"<blue><bold>Starting REAL Batch Audit in {base_dir} using AI endpoint...</bold></blue>")
    ...
    logger.info(f"\n<green><bold>Audit Complete. Total packages audited: {total_audited}</bold></green>")
    if unsafe_packages:
        logger.warning("<red><bold>WARNING: The following packages were flagged as unsafe by the AI:</bold></red>")
        for f, p in unsafe_packages:
            logger.warning(f" - {p} (from {f})")
    else:
        logger.info("<green><bold>All packages appear safe!</bold></green>")
